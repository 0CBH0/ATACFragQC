...
def bedScan(args):
    ...
    ref = ref_raw[(ref_raw["type"] == type_test) & (ref_raw["strand"] != "-") & ref_raw["seq_id"].str.match("^"+chr_list_frag[0]+"$") & (ref_raw["start"] > args.range[0])]
    for term in chr_list_frag[1:]:
        ref = ref.append(ref_raw[(ref_raw["type"] == type_test) & (ref_raw["strand"] != "-") & ref_raw["seq_id"].str.match("^"+term+"$") & (ref_raw["start"] > args.range[0])])
    ...
    ref.loc[ref["strand"] == "+", "start"] -= args.range[0]
    ref.loc[ref["strand"] == "+", "end"] = ref.loc[ref["strand"] == "+", "start"] + args.range[0]*2
    ...
    if dist_count.size > 0:
        factors = np.mean(dist_count[:, list(range(0, args.range[1]))+list(range(tss_range-args.range[1], tss_range))], axis=1)
        factors[factors == 0] = np.mean(factors) if np.mean(factors) > 0 else 1
        dist_count = pd.DataFrame({"V1": list(range(-args.range[0]+args.range[1], args.range[0]-args.range[1]+1)), "V2": list(np.mean(dist_count[:, args.range[1]:(tss_range-args.range[1])] / factors.reshape(len(factors), 1), axis=0))})
...
def main():
    ...
    help_info = "Usage:\nATACFragQC [options] -i <input.bam> -r <reference.gtf>\nArguments:\n"\
        ...
        +"-b, --bg RG,BG\tThe entire range and background of TSS enrichment (default: 1000,100)\n"
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            help_flag = True
        elif opt in ("-i", "--input"):
            ...
            if len(arg.split(",")) == 2:
                arguments.range = list(map(lambda x: int(x), arg.split(",")))
                if arguments.range[0] < arguments.range[1] or arguments.range[0] < 100 or arguments.range[1] < 3:
                    arguments.range = [1000, 100]
